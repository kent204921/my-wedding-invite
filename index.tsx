...
import { Smartphone, PenTool, Unlock, Loader2, Cloud, Share2, Link as LinkIcon, RotateCcw } from 'lucide-react';
import { fetchInvitationData } from './services';

// --- Embedded LanguageToggle Component ---
...
// --- Main App Logic (Moved from App.tsx) ---
...
const App: React.FC = () => {
  ...
  const [previewRsvpSuccess, setPreviewRsvpSuccess] = useState(false);
  ...
  const INITIAL_DATA: InvitationData = {
    ...
    positions: {
      ...
      'rsvp_success_msg_en': { x: 0, y: 0 },
      'rsvp_success_msg_zh': { x: 0, y: 0 },
    },
    elementStyles: {
      ...
      <div className="flex-1 flex overflow-hidden">
        <div className={`flex-1 md:flex-none md:w-[420px] bg-white border-r border-gray-200 shadow-xl z-20 ${mode === 'edit' ? 'block' : 'hidden md:block'}`}>
           <EditorPanel 
             ...
             previewRsvpSuccess={previewRsvpSuccess}
             onTogglePreviewRsvpSuccess={setPreviewRsvpSuccess}
           />
        </div>
          ...
          <div className="relative z-10 w-full h-full max-w-[390px] max-h-[844px] bg-gray-900 rounded-[3rem] shadow-2xl border-[8px] border-gray-900 overflow-hidden ring-1 ring-white/20">
             ...
             <div className="w-full h-full bg-white relative">
                <MobileViewer 
                  ...
                  forceRsvpSuccess={previewRsvpSuccess}
                />
             </div>
          </div>
        </div>
